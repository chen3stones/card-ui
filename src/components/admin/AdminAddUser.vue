<template>
  <el-form :model="mailRegister" :rules="mailRules"  ref="mailRegister" class="demo-ruleForm" size="medium">
    <el-row>
      ...
      <el-col>
        <el-form-item>
          <el-button type="primary" @click="mailSubmit('mailRegister')">立即添加</el-button>
        </el-form-item>
      </el-col>
    </el-row>
  </el-form>
    ...
    data () {
      ...
      var checkMail = (rule, value, callback) => {
        ...
        if (!value) {
          ...
          if (reg.test(value)) {
            axios.get('/api/admin/user/check',
              {
                params: {
                  'account': value
                }
    ...
    methods: {
      ...
      mailSubmit: function (formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.mailRegister.password = this.mailRegister.mail
            axios.post('/api/user/mailSubmit', this.mailRegister)
              .then(result => {
                if (result.data.code === 200) {
                  this.$notify({
                    title: '注册成功',
                    message: ('hi', { style: 'color: teal'}, '默认密码为邮箱号')
                  });
                } else {
